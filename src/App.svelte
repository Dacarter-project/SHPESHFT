<script lang="ts">
  ...
  type TransformDrag = { id: string; kind: 'move' | 'primary' | 'dimension'; before: Transform; beforeSelection?: Record<string, Transform>; baseBounds?: {x:number;y:number;width:number;height:number}; pivot?: { x:number; y:number }; startWorldX: number; startWorldY: number; startDistance?: number; startAngle?: number; axis?: Axis; moved?: boolean };
  ...
  function selectionControls() { if (selectedIds.length === 1 && selectedId) { const object=document.objects[selectedId],b=localBounds(object); return { topLeft:localToWorld(object,b.x,b.y),topRight:localToWorld(object,b.x+b.width,b.y),bottomLeft:localToWorld(object,b.x,b.y+b.height),bottomRight:localToWorld(object,b.x+b.width,b.y+b.height) }; } const b=worldBounds(); return b?{topLeft:{x:b.x,y:b.y},topRight:{x:b.x+b.width,y:b.y},bottomLeft:{x:b.x,y:b.y+b.height},bottomRight:{x:b.x+b.width,y:b.y+b.height}}:null; }
  ...
  function draw() {
    cancelAnimationFrame(frame); frame = requestAnimationFrame(() => {
      ...
      for (const id of document.order) {
        ...
        if (!editMode && id === selectedId) { const controls=selectionControls(); if (controls) { ctx.save(); ctx.setLineDash([]); ctx.lineWidth = 2 / view.scale; ctx.fillStyle = '#f4f1ea'; ctx.strokeStyle = '#111'; const square=20/view.scale; for(const point of [controls.topLeft,controls.topRight]){ctx.fillRect(point.x-square/2,point.y-square/2,square,square);ctx.strokeRect(point.x-square/2,point.y-square/2,square,square);} if(resizeAxis==='width'){ctx.beginPath();ctx.moveTo(controls.topLeft.x-square*.3,controls.topLeft.y);ctx.lineTo(controls.topLeft.x+square*.3,controls.topLeft.y);ctx.stroke();} if(resizeAxis==='height'){ctx.beginPath();ctx.moveTo(controls.topLeft.x,controls.topLeft.y-square*.3);ctx.lineTo(controls.topLeft.x,controls.topLeft.y+square*.3);ctx.stroke();} const nodeRadius=3.5/view.scale;ctx.beginPath();ctx.arc(controls.topRight.x,controls.topRight.y,nodeRadius,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(controls.topRight.x-square*.28,controls.topRight.y-square*.28);ctx.lineTo(controls.topRight.x+square*.28,controls.topRight.y+square*.28);ctx.stroke(); for(const point of [controls.bottomLeft,controls.bottomRight]){ctx.beginPath();ctx.arc(point.x,point.y,10/view.scale,0,Math.PI*2);ctx.fill();ctx.stroke();} ctx.fillStyle='#111';ctx.beginPath();ctx.arc(controls.bottomRight.x,controls.bottomRight.y,4/view.scale,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(controls.bottomLeft.x,controls.bottomLeft.y,2.5/view.scale,0,Math.PI*2);ctx.fill(); if (selectedIds.length === 1) { const axes = axisHandles(object), size = 6 / view.scale; ctx.fillStyle='#f4f1ea'; for (const point of [axes.width, axes.height]) { ctx.fillRect(point.x - size / 2, point.y - size / 2, size, size); ctx.strokeRect(point.x - size / 2, point.y - size / 2, size, size); } } ctx.restore(); } }
  ...
  function pointerDown(event: PointerEvent) {
    ...
    if (!editMode && selectedId) { const selectedObject = document.objects[selectedId], bounds = worldBounds()!, origin = selectedIds.length === 1 ? objectCenter(selectedObject) : { x: bounds.x + bounds.width / 2, y: bounds.y + bounds.height / 2 }, controls=selectionControls(); if(controls&&selectedIds.length===1&&document.objects[selectedId]?.geometry.kind==='path'&&Math.hypot(world.x-controls.topRight.x,world.y-controls.topRight.y)<=22/view.scale){enterEditMode();return;} if(controls && Math.hypot(world.x-controls.bottomLeft.x,world.y-controls.bottomLeft.y)<=24/view.scale){actionsOpen=!actionsOpen;status=actionsOpen?'Selection actions':'Selection';draw();return;} if(controls && selectedIds.length===1 && Math.hypot(world.x-controls.topLeft.x,world.y-controls.topLeft.y)<=22/view.scale){drag={id:selectedId,kind:'dimension',before:selectedObject.transform,pivot:origin,startWorldX:world.x,startWorldY:world.y,startDistance:Math.max(1,Math.hypot(world.x-origin.x,world.y-origin.y)),startAngle:Math.atan2(world.y-origin.y,world.x-origin.x),axis:resizeAxis,moved:false};return;} if (selectedIds.length === 1) { const axes = axisHandles(selectedObject); for (const axis of ['width', 'height'] as const) if (Math.hypot(world.x - axes[axis].x, world.y - axes[axis].y) <= 16 / view.scale) { drag = { id: selectedId, kind: 'dimension', before: selectedObject.transform, pivot: origin, startWorldX: world.x, startWorldY: world.y, startDistance: Math.max(1, Math.hypot(world.x - origin.x, world.y - origin.y)), startAngle: Math.atan2(world.y - origin.y, world.x - origin.x), axis, moved:false }; return; } } const handle = selectionHandle(); if (handle && Math.hypot(world.x - handle.x, world.y - handle.y) <= 26 / view.scale) { drag = { id: selectedId, kind: 'primary', before: selectedObject.transform, beforeSelection: Object.fromEntries(selectedIds.map((id) => [id, document.objects[id].transform])), pivot: origin, startWorldX: world.x, startWorldY: world.y, startDistance: Math.max(1, Math.hypot(world.x - origin.x, world.y - origin.y)), startAngle: Math.atan2(world.y - origin.y, world.x - origin.x), axis: 'both',moved:false }; return; } }
    const hit = [...document.order].reverse().find((id) => hitTest(document.objects[id], world.x, world.y)) ?? null;
    if (hit) { pressStart = { x: event.clientX, y: event.clientY, id: hit }; window.clearTimeout(longPressTimer); longPressTimer = window.setTimeout(() => { multiMode = true; actionsOpen=false; if (!selectedIds.includes(hit)) selectedIds = [hit]; selectedId = hit; drag = null; pressStart = null; status = 'Multi-select · tap shapes · Done'; draw(); }, 450); if (multiMode || event.shiftKey) { selectedIds = selectedIds.includes(hit) ? selectedIds.filter((id) => id !== hit) : [...selectedIds, hit]; selectedId = selectedIds.at(-1) ?? null; actionsOpen=false; draw(); } else { const selection = selectionForObject(hit); if (!selectedIds.includes(hit)){ selectedIds = selection; actionsOpen=false; } selectedId = hit; selectedNodeId = null; drag = { id: hit, kind: 'move', before: document.objects[hit].transform, beforeSelection: Object.fromEntries(selectedIds.map((id) => [id, document.objects[id].transform])), baseBounds: worldBounds() ?? undefined, startWorldX: world.x, startWorldY: world.y }; draw(); } }
    else { pan = { x: event.clientX, y: event.clientY, viewX: view.x, viewY: view.y, moved:false }; draw(); }
  }
  function pointerMove(event: PointerEvent) {
    ...
    if (drag) { const object = document.objects[drag.id]; let transform: Transform;
      ...
      else { const pivot = drag.pivot ?? objectCenter({ ...object, transform: drag.before }), dx = world.x - pivot.x, dy = world.y - pivot.y, factor = Math.max(.08, Math.hypot(dx, dy) / (drag.startDistance ?? 1)), rotationDelta = drag.kind==='primary'?snapped(Math.atan2(dy, dx) - (drag.startAngle ?? 0)):0; drag.moved=drag.moved||Math.hypot(world.x-drag.startWorldX,world.y-drag.startWorldY)>3/view.scale; if (drag.beforeSelection && selectedIds.length > 1) { const objects = { ...document.objects }; for (const id of selectedIds) objects[id] = { ...objects[id], transform: transformAround(objects[id], drag.beforeSelection[id], pivot, factor, rotationDelta) }; document = { ...document, objects }; draw(); return; } if (drag.axis !== 'both') { const local = localCenter(object), cos = Math.cos(-drag.before.rotation), sin = Math.sin(-drag.before.rotation), projectedX = dx * cos - dy * sin, projectedY = dx * sin + dy * cos, start = drag.axis === 'width' ? Math.abs((drag.startWorldX - pivot.x) * cos - (drag.startWorldY - pivot.y) * sin) : Math.abs((drag.startWorldX - pivot.x) * sin + (drag.startWorldY - pivot.y) * cos), axisFactor = Math.max(.08, Math.abs(drag.axis === 'width' ? projectedX : projectedY) / Math.max(1, start)), scaleX = drag.axis === 'width' ? drag.before.scaleX * axisFactor : drag.before.scaleX, scaleY = drag.axis === 'height' ? drag.before.scaleY * axisFactor : drag.before.scaleY; transform = { ...drag.before, x: pivot.x - local.x * scaleX, y: pivot.y - local.y * scaleY, scaleX, scaleY }; } else transform = transformAround(object, drag.before, pivot, factor, rotationDelta); }
      document = { ...document, objects: { ...document.objects, [drag.id]: { ...object, transform } } }; draw();
    }
  }
  function pointerUp(event: PointerEvent) {
    window.clearTimeout(longPressTimer); pressStart = null; pointers.delete(event.pointerId); if (pointers.size < 2) pinch = null; const finishedPan=pan; pan = null; guides = []; if(finishedPan&&!finishedPan.moved&&pointers.size===0&&!multiMode)clearSelection();
    ...
    if (drag) { if(drag.kind==='dimension'&&!drag.moved){resizeAxis=resizeAxis==='both'?'width':resizeAxis==='width'?'height':'both';status=resizeAxis==='both'?'Proportional resize':resizeAxis==='width'?'Width resize':'Height resize';draw();drag=null;return;} if (drag.beforeSelection && selectedIds.length > 1) { const after = Object.fromEntries(selectedIds.map((id) => [id, document.objects[id]])), before = Object.fromEntries(selectedIds.map((id) => [id, { ...document.objects[id], transform: drag!.beforeSelection![id] }])); document = { ...document, objects: { ...document.objects, ...before } }; execute(new ReplaceObjectsCommand(before, after, document.order, document.order)); } else { const object = document.objects[drag.id]; if (JSON.stringify(object.transform) !== JSON.stringify(drag.before)) { const after = object.transform; document = { ...document, objects: { ...document.objects, [drag.id]: { ...object, transform: drag.before } } }; execute(new TransformObjectCommand(drag.id, drag.before, after)); } } drag = null; }
  }
  ...
  <section class="workspace-shell">
    ...
    {:else}
      <nav class="context-dock" class:style-dock={styleMode} aria-label={styleMode ? 'Fill and Stroke' : 'Object actions'}>
        ...
        {:else}
        {#if actionsOpen}
          <button class="done" on:click={() => actionsOpen=false}>Done</button>

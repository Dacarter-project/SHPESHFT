<script lang="ts">
  ...
  function contextualStyle() {
    if (!selectedId) return 'display:none'; const world = selectionControls()?.topRight; if (!world) return 'display:none';
  ...
  function draw() {
    cancelAnimationFrame(frame); frame = requestAnimationFrame(() => {
      ...
      for (const id of document.order) {
        ...
        if (!editMode && id === selectedId) { const controls=selectionControls(); if (controls) { ctx.save(); ctx.setLineDash([]); ctx.lineWidth = 2 / view.scale; ctx.fillStyle = '#f4f1ea'; ctx.strokeStyle = '#111'; const square=20/view.scale; ctx.fillRect(controls.topLeft.x-square/2,controls.topLeft.y-square/2,square,square); ctx.strokeRect(controls.topLeft.x-square/2,controls.topLeft.y-square/2,square,square); if(resizeAxis==='width'){ctx.beginPath();ctx.moveTo(controls.topLeft.x-square*.3,controls.topLeft.y);ctx.lineTo(controls.topLeft.x+square*.3,controls.topLeft.y);ctx.stroke();} if(resizeAxis==='height'){ctx.beginPath();ctx.moveTo(controls.topLeft.x,controls.topLeft.y-square*.3);ctx.lineTo(controls.topLeft.x,controls.topLeft.y+square*.3);ctx.stroke();} for(const point of [controls.bottomLeft,controls.bottomRight]){ctx.beginPath();ctx.arc(point.x,point.y,10/view.scale,0,Math.PI*2);ctx.fill();ctx.stroke();} ctx.fillStyle='#111';ctx.beginPath();ctx.arc(controls.bottomRight.x,controls.bottomRight.y,4/view.scale,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(controls.bottomLeft.x,controls.bottomLeft.y,2.5/view.scale,0,Math.PI*2);ctx.fill(); if (selectedIds.length === 1) { const axes = axisHandles(object), size = 6 / view.scale; ctx.fillStyle='#f4f1ea'; for (const point of [axes.width, axes.height]) { ctx.fillRect(point.x - size / 2, point.y - size / 2, size, size); ctx.strokeRect(point.x - size / 2, point.y - size / 2, size, size); } } ctx.restore(); } }

<script lang="ts">
  ...
  type TransformDrag = { id: string; kind: 'move' | 'primary'; before: Transform; beforeSelection?: Record<string, Transform>; pivot?: { x:number; y:number }; startWorldX: number; startWorldY: number; startDistance?: number; startAngle?: number; axis?: Axis; moved?: boolean };
  ...
  function clearSelection() { selectedId = null; selectedIds = []; selectedNodeId = null; selectedSegment = null; editMode = false; multiMode = false; }
  ...
  function snapMove(dx: number, dy: number) {
    const bounds = worldBounds(); if (!bounds) return { dx, dy }; const threshold = 8 / view.scale, others = document.order.filter((id) => !selectedIds.includes(id)).map((id) => worldBounds([id])).filter(Boolean) as Array<{x:number;y:number;width:number;height:number}>;
  ...
  function draw() {
    cancelAnimationFrame(frame); frame = requestAnimationFrame(() => {
      ...
      if (guides.length) { ctx.save(); ctx.strokeStyle = '#2457ff'; ctx.lineWidth = 1 / view.scale; ctx.setLineDash([5 / view.scale, 5 / view.scale]); for (const guide of guides) { ctx.beginPath(); if (guide.axis === 'x') { ctx.moveTo(guide.value, 0); ctx.lineTo(guide.value, document.workspace.height); } else { ctx.moveTo(0, guide.value); ctx.lineTo(document.workspace.width, guide.value); } ctx.stroke(); } ctx.restore(); }
      for (const id of document.order) {
        ...
        const b = localBounds(object); if (selectedIds.length === 1) { ctx.save(); applyObjectTransform(ctx, object); ctx.strokeStyle = '#2457ff'; ctx.lineWidth = 1.5 / view.scale; ctx.setLineDash([6 / view.scale, 5 / view.scale]); ctx.strokeRect(b.x, b.y, b.width, b.height); ctx.restore(); }
        ...
        if (editMode && id === selectedId && object.geometry.kind === 'path') {
          ctx.save(); ctx.setLineDash([]); ctx.lineWidth = 1.5 / view.scale; ctx.strokeStyle = '#2457ff';
          ...
          for (const node of object.geometry.nodes) {
            ...
            const size = (node.id === selectedNodeId ? 11 : 8) / view.scale; ctx.fillStyle = node.id === selectedNodeId ? '#2457ff' : '#fff'; ctx.fillRect(anchor.x - size / 2, anchor.y - size / 2, size, size); ctx.strokeRect(anchor.x - size / 2, anchor.y - size / 2, size, size);
          }
          ctx.restore();
        }
      }
      if (selectedIds.length > 1) { const b = worldBounds(); if (b) { ctx.save(); ctx.strokeStyle = '#2457ff'; ctx.lineWidth = 1.5 / view.scale; ctx.setLineDash([6 / view.scale, 5 / view.scale]); ctx.strokeRect(b.x, b.y, b.width, b.height); ctx.restore(); } }
  ...
  function pointerMove(event: PointerEvent) {
    ...
    if (drag) { const object = document.objects[drag.id]; let transform: Transform;
      if (drag.kind === 'move') { const snappedMove = snapMove(world.x - drag.startWorldX, world.y - drag.startWorldY); if (drag.beforeSelection && selectedIds.length > 1) { const objects = { ...document.objects }; for (const id of selectedIds) { const before = drag.beforeSelection[id]; objects[id] = { ...objects[id], transform: { ...before, x: before.x + snappedMove.dx, y: before.y + snappedMove.dy } }; } document = { ...document, objects }; drag.moved = true; draw(); return; } transform = { ...drag.before, x: drag.before.x + snappedMove.dx, y: drag.before.y + snappedMove.dy }; drag.moved = true; }
